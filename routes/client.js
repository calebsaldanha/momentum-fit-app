...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const statsQuery = `SELECT COUNT(*) as count FROM checkins WHERE user_id = $1 AND date >= date_trunc('week', CURRENT_DATE)`;
        const statsRes = await db.query(statsQuery, [userId]);
        const workoutsThisWeek = statsRes.rows[0].count;

        const nextWorkoutQuery = `SELECT id, title, description, muscle_group FROM workouts WHERE client_id = $1 AND status = 'active' ORDER BY created_at DESC LIMIT 1`;
        const workoutRes = await db.query(nextWorkoutQuery, [clientProfile.id]);
        const nextWorkout = workoutRes.rows[0] || null;

        const subQuery = `SELECT p.name, s.end_date FROM subscriptions s JOIN plans p ON s.plan_id = p.id WHERE s.user_id = $1 AND s.status = 'active' LIMIT 1`;
...
router.get('/workouts', async (req, res) => {
    try {
        ...
        const workouts = await db.query(`
            ...
            FROM workouts w WHERE w.client_id = $1 ORDER BY w.status ASC, w.created_at DESC
        `, [clientRes.rows[0].id]);
...
// Rota POST para atualizar Perfil (Anamnese)
...
router.get('/financial', async (req, res) => {
    ...
    res.render('pages/client-financial', { subscription: null, transactions: [], user: req.session.user });
});
