...
router.get('/dashboard', requireClient, async (req, res) => {
    try {
        ...
        // Correção: Busca dados do perfil (Peso, Altura) para exibir no dashboard
...
router.get('/workouts', requireClient, async (req, res) => {
    ...
    } catch (err) { res.status(500).render('pages/error', { message: 'Erro ao carregar treinos.' }); }
});
...
router.post('/change-password', requireClient, async (req, res) => {
    ...
    if (new_password !== confirm_password) return res.redirect('/client/profile?error=Senhas não conferem');

    try {
        const userRes = await pool.query("SELECT * FROM users WHERE id = $1", [req.session.user.id]);
        const user = userRes.rows[0];
        if (!await bcrypt.compare(current_password, user.password)) return res.redirect('/client/profile?error=Senha incorreta');
        ...
        res.redirect('/client/profile?success=Senha alterada');
    } catch (err) { res.status(500).render('pages/error', { message: 'Erro ao alterar senha.' }); }
});
