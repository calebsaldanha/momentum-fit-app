...
router.post('/users/delete/:id', async (req, res) => {
    try {
        ...
        if(targetId == req.session.user.id) {
            req.flash('error', 'Você não pode se excluir.');
            return res.redirect('/admin/users');
        }
        
        await db.query('BEGIN');
        // Remover dependencias (Checkins, Workouts, Subscriptions, Payments, Messages, Clients)
        await db.query('DELETE FROM checkins WHERE user_id = $1', [targetId]);
        await db.query('DELETE FROM workout_exercises WHERE workout_id IN (SELECT id FROM workouts WHERE client_id IN (SELECT id FROM clients WHERE user_id = $1))', [targetId]);
        await db.query('DELETE FROM workouts WHERE client_id IN (SELECT id FROM clients WHERE user_id = $1)', [targetId]);
        await db.query('DELETE FROM subscriptions WHERE user_id = $1', [targetId]);
        await db.query('DELETE FROM payments WHERE user_id = $1', [targetId]);
        await db.query('DELETE FROM messages WHERE sender_id = $1 OR receiver_id = $1', [targetId]);
        await db.query('DELETE FROM clients WHERE user_id = $1', [targetId]);
