...
app.use((req, res, next) => {
    ...
    res.locals.title = 'Momentum Fit'; // Título padrão para evitar ReferenceError
    next();
});
...
app.use('/workouts', require('./routes/workouts')); // Rota central de treinos
app.use('/chat', require('./routes/chat'));

// Tratamento de Erros
app.use((err, req, res, next) => {
    if (err.code === 'EBADCSRFTOKEN') {
        return res.status(403).render('pages/error', { title: 'Erro de Segurança', message: 'Sessão inválida.' });
    }
